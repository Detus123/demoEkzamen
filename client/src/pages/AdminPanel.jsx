...
export default function AdminPanel() {
  ...
  const getStatusOptions = (current) => [
    ...
    { value: 'in_progress', label: 'Банкет назначен', disabled: current === 'in_progress' },
    { value: 'completed', label: 'Банкет завершен', disabled: current === 'completed' },
  ]
  return (
    <Layout>
      <div className="card">
        <div className="flex-between" style={{ marginBottom: '20px', flexWrap: 'wrap', gap: '12px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            {['all', 'new', 'in_progress', 'completed'].map(f => (
              ...
              >
                {f === 'all' ? 'Все' : f === 'new' ? 'Новые' : f === 'in_progress' ? 'Назначенные' : 'Завершённые'}
              </button>
            ))}
          </div>
        </div>
                  ...
                  {paginated.map(app => (
                    <tr key={app.id} className="hover-pulse">
                      ...
                      <td>
                        ...
                        <div style={{ fontSize: '11px', color: '#DAA520' }}>{app.login}</div>
                      </td>
                      <td>{app.hall_name}</td>
                      ...
                      <td>
                        <button
                          onClick={() => {
                            navigator.clipboard.writeText(`${app.login} | ${app.hall_name}`);
