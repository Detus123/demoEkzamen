...
const pool = new Pool({
  ...
  database: 'banketNET_db',
  password: process.env.DB_PASS || '',
  port: 5432,
})
const SECRET = 'exem_secret_2026'
...
app.post('/api/applications', auth, async (req, res) => {
  const { hall_name, start_date, payment_method } = req.body
  const { rows } = await pool.query(
    'INSERT INTO applications (user_id, hall_name, start_date, payment_method) VALUES ($1,$2,$3,$4) RETURNING *',
    [req.user.id, hall_name, start_date, payment_method]
  )
  res.json(rows[0])
})
